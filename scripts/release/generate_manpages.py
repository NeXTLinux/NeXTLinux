...
default_output_path = '../../nextlinux/doc/man'
...
def check_nextlinux():
    if os.system('nextlinux --version') != 0:
        print 'Must have nextlinux installed first.'
        exit(2)
...
def generate_manpages(command, recurse=False, version=None):
    ...
    if os.system(' '.join(command) + ' --extended-help') != 0:
        ...
        if os.system('help2man --version-string="' + version + '" -N "' +
                     ' '.join(command) + '" > ' + outfile) != 0:
            print 'Failed on ' + str(command) + ' SKIPPING'
    else:
        if os.system(
                'help2man --help-option="--extended-help" --version-string="' +
                version + '" -N --no-discard-stderr "' + ' '.join(command) +
                '" > ' + outfile) != 0:
            print 'Failed on ' + str(command) + ' SKIPPING'
...
if __name__ == '__main__':
    ...
    print 'Building manpages from help output of nextlinux commands'
    generate_manpages(['nextlinux'], recurse=True)
