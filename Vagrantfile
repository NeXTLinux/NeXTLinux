...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
     ...
     echo "export PATH=~/.local/bin:$PATH" > /etc/profile.d/nextlinux.sh
     chmod 755 /etc/profile.d/nextlinux.sh
     docker pull nginx:latest
     source /etc/profile.d/nextlinux.sh
     export LC_CTYPE="en_US.UTF-8"
     export LC_ALL=C
     git clone https://github.com/nextlinux/nextlinux.git
     cd nextlinux
