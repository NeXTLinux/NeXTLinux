...
class Navigator(object):
    ...
    def __init__(self, nextlinux_config, imagelist, allimages):
        self.config = nextlinux_config
        self.allimages = allimages
        #self.nextlinux_datadir = self.config['image_data_store']
        self.images = list()
    ...
    def add_images(self, imagelist):
        newimages = nextlinux_utils.image_context_add(
            imagelist,
            self.allimages,
            docker_cli=contexts['docker_cli'],
            tmproot=self.config['tmpdir'],
    ...
    def generate_reports(self):
        ...
        for imageId in self.images:
            ...
            if image.meta['imageId'] != baseId:
                diffdata = nextlinux_utils.diff_images(image.meta['imageId'],
                                                     baseId)
    ...
    def get_dockerfile_contents(self):
        ...
        for imageId in self.images:
            ...
            record['result']['header'] = [
                'Image_Id', 'Mode', 'Dockerfile_Contents'
            ]
            record['result']['rows'] = list()
            (dbuf, mode) = image.get_dockerfile_contents()
            record['result']['rows'].append(
                [image.meta['shortId'], mode, dbuf])
    ...
    def get_familytree(self):
        ...
        for imageId in self.images:
            ...
            record = {'result': {}}
            record['result']['header'] = [
                'Image_Id', 'Current_Repo_Tags', 'Past_Repo_Tags', 'Image_Type'
            ]
            record['result']['rows'] = list()
            for fid in image.get_familytree():
                ...
                if usertype == "nextlinuxbase":
                    userstr = "Nextlinux Base Image"
                elif usertype == "base":
                    userstr = "Base"
                elif usertype == "oldnextlinuxbase":
                    userstr = "Previous Nextlinux Base Image"
                elif usertype == "user":
                    userstr = "User Image"
                else:
                ...
                record['result']['rows'].append(
                    [fidstr, curr_tags, past_tags, userstr])
    ...
    def get_taghistory(self):
        ...
        for imageId in self.images:
            ...
            for tagtup in image.get_tag_history():
                ...
                record['result']['rows'].append(
                    [image.meta['shortId'], tstr, tagstr])
    ...
    def unpack(self, destdir='/tmp'):
        ...
        for imageId in self.images:
            ...
            if not outdir:
                ...
                self._logger.debug("Unpacked image " + image.meta['shortId'] +
                                   " in " + outdir)
                ret[imageId] = outdir
        return (ret)
    ...
    def find_query_command(self, action):
        ...
        try:
            path_overrides = [
                '/'.join([self.config['user_scripts_dir'], 'queries'])
            ]
            if self.config['extra_scripts_dir']:
                path_overrides = path_overrides + [
                    '/'.join([self.config['extra_scripts_dir'], 'queries'])
                ]

            cmdobj = scripting.ScriptExecutor(path='/'.join(
                [self.config['scripts_dir'], 'queries']),
                                              script_name=action,
                                              path_overrides=path_overrides)
            cmd = cmdobj.thecmd
            mode = 'query'
            ...
            try:
                path_overrides = [
                    '/'.join(
                        [self.config['user_scripts_dir'], 'multi-queries'])
                ]
                if self.config['extra_scripts_dir']:
                    path_overrides = path_overrides + [
                        '/'.join([
                            self.config['extra_scripts_dir'], 'multi-queries'
                        ])
                    ]
                cmdobj = scripting.ScriptExecutor(
                    path='/'.join(
                        [self.config['scripts_dir'], 'multi-queries']),
                    script_name=action,
                    path_overrides=path_overrides)
                cmd = cmdobj.thecmd
                mode = 'multi-query'
    ...
    def execute_query(self, imglist, se, params):
        ...
        outputdir = '/'.join([
            self.config['nextlinux_data_dir'], "querytmp",
        ...
        nextlinux_utils.write_plainfile_fromlist(imgfile, imglist)
        ...
        try:
            ...
            try:
                ...
                try:
                    frows = nextlinux_utils.read_kvfile_tolist(ofile)
                    header = frows[0]
                    rowlen = len(header)
                    for row in frows[1:]:
                        if len(row) != rowlen:
                ...
                if warnfile:
                    try:
                        meta['warns'] = nextlinux_utils.read_plainfile_tolist(
                            warnfile)
                    except:
                        pass
    ...
    def format_query_manifest_record(self, command, status, returncode,
                                     timestamp, qtype, output, csum):
        ret = {
            "status": status,
    ...
    def list_query_commands(self, command=None):
        ...
        query_manifest = self.nextlinuxDB.load_query_manifest()
        ...
        if command:
            try:
                ...
                if rc:
                    ...
                    csum = se.csum()
                    if commandpath in query_manifest and csum != "N/A" and csum == query_manifest[
                            commandpath]['csum']:
                        self._logger.debug(
                            "skipping query help refresh run (" +
                            str(command) + "): no change in query")
                        cmd = query_manifest[commandpath]['command']
                        rc = query_manifest[commandpath]['returncode']
                        sout = query_manifest[commandpath]['output']
                    else:
                        (cmd, rc, sout) = se.execute(capture_output=True,
                                                     cmdline='help')

                    if rc == 0:
                        query_manifest[
                            commandpath] = self.format_query_manifest_record(
                                cmd, "SUCCESS", rc, time.time(), "N/A", sout,
                                csum)
                        record['result']['rows'].append([command, sout])
                    else:
                        query_manifest[
                            commandpath] = self.format_query_manifest_record(
                                cmd, "FAIL", rc, time.time(), "N/A", "N/A",
                                csum)

            except Exception as err:
                self._logger.debug("cannot execute (" + str(command) +
                                   "): skipping in list")
            ...
            if self.config['user_scripts_dir']:
                paths.append('/'.join(
                    [self.config['user_scripts_dir'], 'queries']))
                paths.append('/'.join(
                    [self.config['user_scripts_dir'], 'multi-queries']))

            if self.config['extra_scripts_dir']:
                paths.append('/'.join(
                    [self.config['extra_scripts_dir'], 'queries']))
                paths.append('/'.join(
                    [self.config['extra_scripts_dir'], 'multi-queries']))
            ...
            for dd in paths:
                ...
                for d in os.listdir(dd):
                    ...
                    try:
                        (rc, command_type,
                         se) = self.find_query_command(command)
                        if rc:
                            commandpath = se.get_script()
                            csum = se.csum()
                            if commandpath in query_manifest and csum != "N/A" and csum == query_manifest[
                                ...
                                (cmd, rc,
                                 sout) = se.execute(capture_output=True,
                                                    cmdline='help')

                            if rc == 0:
                                query_manifest[
                                    commandpath] = self.format_query_manifest_record(
                                        cmd, "SUCCESS", rc, time.time(),
                                        command_type, sout, csum)
                                record['result']['rows'].append(
                                    [command, sout])
                            else:
                                query_manifest[
                                    commandpath] = self.format_query_manifest_record(
                                        cmd, "FAIL", rc, time.time(),
                                        command_type, "N/A", csum)

                    except Exception as err:
                        self._logger.debug("cannot execute (" + str(command) +
                                           "): skipping in list")

        self.nextlinuxDB.save_query_manifest(query_manifest)
    ...
    def run_query(self, query):
        ...
        if not rc:
            self._logger.error(
                "cannot find query command in Nextlinux query directory: " +
                action + "(.py|.sh)")
            return (False)
