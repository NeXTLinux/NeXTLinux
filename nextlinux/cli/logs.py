...
#     'normal': quiet + status messages from nextlinux.console logger to stderr as well as ERRORs
...
class LoggerNamePrefixFilter(logging.Filter):
    """
    Filters out all non-Nextlinux records by logger name.
    """
...
def init_output_formatters(output_verbosity='normal',
    ...
    if output_verbosity not in console_verbosity_options:
        raise ValueError('output_verbosity must be one of: %s' %
                         console_verbosity_options.keys())

    # Initialize debug log file, 'nextlinux-debug.log'. This log has stack-traces and is expected to be human read
    ...
    if output_verbosity == 'quiet':
        ...
        stderr_handler.addFilter(
            LoggerNamePrefixFilter(prefix='nextlinux',
...
if __name__ == '__main__':
    ...
    log = logging.getLogger('nextlinux.test')
