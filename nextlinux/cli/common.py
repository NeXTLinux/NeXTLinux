...
from nextlinux.cli import logs
...
def extended_help_option(extended_help=None, *param_decls, **attrs):
    ...
    def decorator(f):
        ...
        return click.option(*(param_decls or ('--extended-help', )),
                            **attrs)(f)
...
def init_output_format(use_json=False,
                       use_plain=False,
                       use_debug=False,
                       use_verbose=False,
                       use_quiet=False,
                       log_filepath=None,
                       debug_log_filepath=None):
    global formatter
    ...
    logs.init_output_formatters(output_verbosity=level,
                                logfile=log_filepath,
                                debug_logfile=debug_log_filepath)


def nextlinux_print_err(msg):
    exc = sys.exc_info()
...
def nextlinux_print(msg, do_formatting=False):
    """
...
def build_image_list(config,
    ...
    if not image and not (imagefile or all_local):
        raise click.BadOptionUsage(
            'No input found for image source. One of <image>, <imagefile>, or <all> must be specified'
        )

    if image and imagefile:
        raise click.BadOptionUsage(
            'Only one of <image> and <imagefile> can be specified')
    ...
    if imagefile:
        filelist = nextlinux_utils.read_kvfile_tolist(imagefile)
    ...
    if all_local:
        ...
        if docker_cli:
            for f in docker_cli.images(all=True,
                                       quiet=True,
            ...
            raise Exception(
                "Could not load any images from local docker host - is docker running?"
            )

    if include_allnextlinux:
        ret = contexts['nextlinux_db'].load_all_images().keys()
