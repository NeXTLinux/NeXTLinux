...
import nextlinux.nextlinux_auth
...
def get_feed_list():
    ...
    feedurl = contexts['nextlinux_config']['feeds_url']
    feed_timeout = contexts['nextlinux_config']['feeds_conn_timeout']
    feed_maxretries = contexts['nextlinux_config']['feeds_max_retries']
    ...
    while not done:
        record = nextlinux.nextlinux_auth.nextlinux_auth_get(
...
def create_feed(feed):
    ...
    return (contexts['nextlinux_db'].create_feed(feed))
...
def create_feedgroup(feed, group):
    ...
    return (contexts['nextlinux_db'].create_feedgroup(feed, group))
...
def sync_feedmeta(default_sublist=['vulnerabilities']):
    ...
    try:
        feedmeta = load_nextlinux_feedmeta()
        ...
        if feeds:
            for feedrecord in feeds:
                ...
                if groups:
                    ...
                    ret['text'] = "WARN: cannot get list of groups from feed: " + str(
                        feed)
        ...
        save_nextlinux_feedmeta(feedmeta)
...
def feed_group_data_exists(feed, group, datafile):
    ...
    ) and 'datafiles' in feedmeta[feed]['groups'][
...
def sync_feeds(force_since=None, do_combine=False):
    ...
    try:
        for feed in feedmeta.keys():
            if feedmeta[feed]['subscribed']:
                ...
                if groups:
                    for group in groups:
                        ...
                        now = time.strftime("%Y-%m-%d",
                                            time.gmtime(updatetime))

                        datafilename = "data_" + since + "_to_" + now + ".json"
                        if feed_group_data_exists(feed, group, datafilename):
                            _logger.info("\tskipping group data: " +
                                         str(group) + ": already synced")
                        else:
                            _logger.info("\tsyncing group data: " +
                                         str(group) + ": ...")
                            ...
                            success, data = get_group_data(feed,
                                                           group,
                                                           since=since,
                                                           uniq_key=uniq_key)
                            if success:
                                rc = save_nextlinux_feed_group_data(
                                    feed, group, datafilename, data)

                                group_meta['prev_update'] = sincets
                                group_meta['last_update'] = updatetime
                                if datafilename not in group_meta['datafiles']:
                                    group_meta['datafiles'].append(
                                        datafilename)

                                # finally, do any post download feed/group handler actions
                                rc, msg = handle_nextlinux_feed_post(feed, group)
                            else:
                ...
                _logger.info("skipping data sync for unsubscribed feed (" +
                             str(feed) + ") ...")
    ...
    if not save_nextlinux_feedmeta(feedmeta):
...
def check():
    if not load_nextlinux_feedmeta():
        return (
            False,
            "feeds are not initialized: please run 'nextlinux feeds sync' and try again"
        )

    if not load_nextlinux_feeds_list():
        return (
            False,
            "feeds list is empty: please run 'nextlinux feeds sync' and try again"
        )

    return (True, "success")


def load_nextlinux_feeds_list():
    ret = []
    feedmeta = load_nextlinux_feedmeta()
    if feedmeta:
        ret = feedmeta.values()
...
def load_nextlinux_feed_groups_list(feed):
    ret = []
    feedmeta = load_nextlinux_feedmeta()
    if feedmeta:
...
def load_nextlinux_feed_group_datameta(feed, group):
    ret = {}
    feedmeta = load_nextlinux_feedmeta()
    if feedmeta:
        if feed in feedmeta.keys() and group in feedmeta[feed]['groups'].keys(
...
def load_nextlinux_feedmeta():
    return (contexts['nextlinux_db'].load_feedmeta())


def save_nextlinux_feedmeta(feedmeta):
    return (contexts['nextlinux_db'].save_feedmeta(feedmeta))


def subscribe_nextlinux_feed(feed, user_tier=0):
    success = True
        ...
        if user_tier >= int(feedmeta[feed]['access_tier']):
            if not feedmeta[feed]['subscribed']:
                ...
                if not save_nextlinux_feedmeta(feedmeta):
                    msg = str(
                        feed
                    ) + ": failed to subscribe to feed (check debug output)."
                    success = False
        else:
            msg = 'Current user does not have sufficient access tier to subscribe to feed {0}. Current tier is {1}, must be {2} to access feed'.format(
                feed, user_tier, feedmeta[feed]['access_tier'])
            success = False
    else:
        msg = "cannot find specified feed (" + str(
            feed) + "): please review the feeds list and try again"
        success = False
    return (success, msg)


def unsubscribe_nextlinux_feed(feed):
    success = True
    ...
    if feed in feedmeta:
        if feedmeta[feed]['subscribed']:
            ...
            if not save_nextlinux_feedmeta(feedmeta):
                msg = str(
                    feed
                ) + ": failed to unsubscribe to feed (check debug output)."
                success = False
    else:
        msg = "cannot find specified feed (" + str(
            feed) + "): please review the feeds list and try again"
...
def save_nextlinux_feed_group_data(feed, group, datafile, data):
    return (contexts['nextlinux_db'].save_feed_group_data(
        feed, group, datafile, data))


def load_nextlinux_feed_group_data(feed, group, datafile):
    return (contexts['nextlinux_db'].load_feed_group_data(feed, group, datafile))


def delete_nextlinux_feed_group_data(feed, group, datafile):
    return (contexts['nextlinux_db'].delete_feed_group_data(
        feed, group, datafile))


def load_nextlinux_feed(feed, group, ensure_unique=False):
    ...
    if not feedmeta:
        ...
        ret['msg'] = "not currently subscribed to feed (" + str(
            feed) + "): please subscribe, sync, and try again"
    else:
        ...
        if datameta and 'datafiles' in datameta:
            ...
            for datafile in revfiles:
                thelist = load_nextlinux_feed_group_data(feed, group, datafile)
            ...
            ret['msg'] = "no data exists for given feed/group (" + str(
                feed) + "/" + str(group) + ")"

    return (ret)


def delete_nextlinux_feed(feed):
    feedmeta = load_nextlinux_feedmeta()
    if feed in feedmeta.keys():
        if not feedmeta[feed]['subscribed']:
            try:
                ...
                save_nextlinux_feedmeta(feedmeta)
                contexts['nextlinux_db'].delete_feed(feed)
            except Exception as err:
                _logger.warn(
                    "could not complete delete of feed - message from service: "
                    + str(err))
...
# TODO wip
def handle_nextlinux_feed_pre(feed):
    ret = True
    msg = ""
    feedmeta = load_nextlinux_feedmeta()
    if feed in feedmeta:
        if feed == 'vulnerabilities':
            if not feedmeta[feed]['subscribed']:
                rc, msg = subscribe_nextlinux_feed(feed)
                ret = rc
    return (ret, msg)


def handle_nextlinux_feed_post(feed, group):
    ret = True
    msg = ""
    if feed == 'imagedata':
        # handler
        d = load_nextlinux_feed(feed, group)
        if d and d['success']:
            for record in d['data']:
                if 'redirecturl' in record.keys():
                    for tag in record['redirecturl'].keys():
                        ...
                        imageId = tag
                        if not contexts['nextlinux_db'].is_image_present(
                                imageId):
                            try:
                                ...
                                if r.status_code == 200:
                                    ...
                                    for imagedata in data:
                                        imagerecord = imagedata['image'][
                                            'imagedata']
                                        _logger.info(
                                            "\t\tpopulating nextlinux DB with image data: "
                                            + imageId)
                                        contexts['nextlinux_db'].save_image_new(
                                            imageId, report=imagerecord)
                            except Exception as err:
                                _logger.error("exception: " + str(err))
                                ret = False
                                msg = "failed to download/import image: " + imageId
                        else:
                            _logger.info("\t\tskipping: " + str(imageId) +
                                         ": already in DB")
    else:
        # no handler
        pass
...
def handle_datafile_combine():
    ...
    for feed in feedmeta.keys():
        if 'groups' in feedmeta[feed] and 'subscribed' in feedmeta[
                feed] and feedmeta[feed]['subscribed']:
            _logger.info("combining data for feed (" + str(feed) + ") ...")
            for group in feedmeta[feed]['groups']:
                rawdata = load_nextlinux_feed(feed, group, ensure_unique=False)
                data = rawdata['data']
                uniqhash = {}
                uniq = list()
                collisions = 0
                ...
                rawdata.clear()
                _logger.info("\tprocessing group data: " + str(group) +
                             ": removed " + str(collisions) +
                ...
                rc = save_nextlinux_feed_group_data(feed, group, datafilename,
                                                  uniq)
                if rc:
                    if 'datafiles' in feedmeta[feed]['groups'][group]:
                        for dfile in feedmeta[feed]['groups'][group][
                                'datafiles']:
                            if dfile != datafilename:
                                delete_nextlinux_feed_group_data(
                                    feed, group, dfile)

                    feedmeta[feed]['groups'][group]['datafiles'] = [
                        datafilename
                    ]
                    save_nextlinux_feedmeta(feedmeta)
