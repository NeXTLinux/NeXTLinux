...
def check():
    if not load_policymeta():
        return (
            False,
            "policys are not initialized: please run 'nextlinux policys sync' and try again"
        )
...
def sync_policymeta(bundlefile=None, outfile=None):
    ...
    policyurl = contexts['nextlinux_config']['policy_url']
    policy_timeout = contexts['nextlinux_config']['policy_conn_timeout']
    policy_maxretries = contexts['nextlinux_config']['policy_max_retries']
    ...
    if bundlefile:
        ...
        try:
            ...
            ret['text'] = "synced policy bundle cannot be read/is not valid JSON: exception - " + str(
                err)
            return (False, ret)
    else:
        record = nextlinux_auth.nextlinux_auth_get(contexts['nextlinux_auth'],
                                               policyurl,
                                               timeout=policy_timeout,
                                               retries=policy_maxretries)
        if record['success']:
            try:
                bundleraw = json.loads(record['text'])
                policymeta = bundleraw['bundle']
            except Exception as err:
                ret['text'] = 'failed to parse bundle response from service - exception: ' + str(
                    err)
                return (False, ret)
        else:
            _logger.debug(
                "failed to download policybundle: message from server - " +
                str(record))
            themsg = "unspecificied failure while attempting to download bundle from nextlinux.io"
            try:
                if record['status_code'] == 404:
                    themsg = "no policy bundle found on nextlinux.io - please create and save a policy using the policy editor in nextlinux.io and try again"
                elif record['status_code'] == 401:
                    themsg = "cannot download a policy bundle from nextlinux.io - current user does not have access rights to download custom policies"
            except Exception as err:
                themsg = "exception while inspecting response from server - exception: " + str(
                    err)
    ...
    if not verify_policy_bundle(bundle=policymeta):
        _logger.debug("downloaded policy bundle failed to verify: " +
                      str(policymeta))
    ...
    if outfile:
        if outfile != '-':
            try:
                ...
                ret['text'] = "could not write downloaded policy bundle to specified file (" + str(
                    outfile) + ") - exception: " + str(err)
                return (False, ret)
    else:
        if not contexts['nextlinux_db'].save_policymeta(policymeta):
            ret['text'] = "cannot get list of policies from service\nMessage from server: " + record[
                'text']
            return (False, ret)
...
def load_policymeta(policymetafile=None):
    ...
    if policymetafile:
        ...
        ret = contexts['nextlinux_db'].load_policymeta()
        if not ret:
            # use the system default
            default_policy_bundle_file = os.path.join(
                contexts['nextlinux_config'].config_dir,
            ...
            try:
                if os.path.exists(default_policy_bundle_file):
                    ...
                    raise Exception("no such file: " +
                                    str(default_policy_bundle_file))
            except Exception as err:
                _logger.warn("could not load default bundle (" +
                             str(default_policy_bundle_file) +
                             ") - exception: " + str(err))
                raise err
...
def save_policymeta(policymeta):
    return (contexts['nextlinux_db'].save_policymeta(policymeta))
...
# Convert
def convert_to_policy_bundle(name="default",
                             version=default_bundle_version,
                             policy_file=None,
                             policy_version=default_policy_version,
                             whitelist_files=[],
                             whitelist_version=default_whitelist_version):
    policies = {}
    ...
    m = create_mapping(map_name="default",
                       policy_name=policies.keys()[0],
                       whitelists=whitelists.keys(),
                       repotagstring='*/*:*')
    mappings.append(m)

    bundle = create_policy_bundle(name='default',
                                  policies=policies,
...
# C
def create_policy_bundle(name=None,
                         version=default_bundle_version,
                         policies={},
                         policy_version=default_policy_version,
                         whitelists={},
    ...
    for f in policies:
        el = {'version': policy_version, 'id': f, 'name': f, 'rules': []}

        el['rules'] = unformat_policy_data(policies[f])
        ret['policies'].append(el)

    for f in whitelists:
        el = {'version': whitelist_version, 'id': f, 'name': f, 'items': []}
    ...
    _logger.debug("created bundle: (" + str(name) + ") : " +
                  json.dumps(ret.keys(), indent=4))
    return (ret)
...
def verify_policy_bundle(bundle={}):
    ...
    try:
        bundle_schema_file = os.path.join(
            contexts['nextlinux_config']['pkg_dir'], 'schemas',
            'nextlinux-bundle.schema')
    except:
        from pkg_resources import Requirement, resource_filename
        bundle_schema_file = os.path.join(resource_filename("nextlinux", ""),
                                          'schemas', 'nextlinux-bundle.schema')
    ...
    try:
        ...
        _logger.error("could not load bundle schema: " +
                      str(bundle_schema_file))
        return (False)
    ...
    if not bundle_schema:
        ...
        try:
            ...
            _logger.error("could not validate bundle against schema: " +
                          str(err))
            return (False)
...
# U
def update_policy_bundle(bundle={},
                         name=None,
                         policies={},
                         whitelists={},
                         mappings={}):
    if not verify_policy_bundle(bundle=bundle):
        raise Exception(
            "input bundle is incomplete - cannot update bad bundle: " +
            json.dumps(bundle, indent=4))

    ret = {}
    ret.update(bundle)

    new_bundle = create_policy_bundle(name=name,
                                      policies=policies,
                                      whitelists=whitelists,
                                      mappings=mappings)
...
def write_policy_bundle(bundle_file=None, bundle={}):
    ...
    if not verify_policy_bundle(bundle=bundle):
        raise Exception("cannot verify input policy bundle, skipping write: " +
                        str(bundle_file))
...
# C
def create_mapping(map_name=None,
                   policy_name=None,
                   whitelists=[],
                   repotagstring=None):
    ret = {}
    ...
    image_info = nextlinux_utils.get_all_image_info(repotagstring)
...
def read_whitelist(name=None, file=None, version=default_whitelist_version):
    ...
    if file:
        ...
        wdata = nextlinux_utils.read_plainfile_tolist(file)
...
def unformat_whitelist_data(wldata):
    ...
    for wlitem in whitelists:
        ...
        el = {'gate': gate, 'trigger_id': triggerId, 'id': str(uuid.uuid4())}
        ret.append(el)
    return (ret)
...
def format_whitelist_data(wldata):
    ...
    if wldata['version'] == default_whitelist_version:
        ...
        raise Exception(
            "detected whitelist version format in bundle not supported: " +
            str(version))
...
def read_policy(name=None, file=None, version=default_bundle_version):
    ...
    pdata = nextlinux_utils.read_plainfile_tolist(file)
...
def format_policy_data(poldata):
    ...
    if poldata['version'] == default_policy_version:
        for item in poldata['rules']:
            polline = ':'.join(
                [item['gate'], item['trigger'], item['action'], ""])

            if 'params' in item:
                for param in item['params']:
                    polline = polline + param['name'] + '=' + param[
                        'value'] + " "
            ret.append(polline)

    else:
        raise Exception(
            "detected policy version format in bundle not supported: " +
            str(version))
...
def run_bundle(nextlinux_config=None,
               bundle={},
               image=None,
               matchtags=[],
               stateless=False,
    ...
    if not nextlinux_config or not bundle or not image:
        raise Exception("input error")

    if not verify_policy_bundle(bundle=bundle):
        raise Exception("input bundle does not conform to bundle schema")

    imageId = nextlinux_utils.discover_imageId(image)
    digests = []
...
def get_mapping_actions(image=None, imageId=None, in_digests=[], bundle={}):
    ...
    image_info = nextlinux_utils.get_all_image_info(image)
    ...
    for m in bundle['mappings']:
        ...
        for image_info in image_infos:
            ...
            if fulltag != 'N/A':
                tinfo = nextlinux_utils.parse_dockerimage_string(fulltag)
                if 'tag' in tinfo and tinfo['tag']:
                    tag = tinfo['tag']

            for t in [image, fulltag]:
                tinfo = nextlinux_utils.parse_dockerimage_string(t)
            ...
            for d in image_info['digests']:
                dinfo = nextlinux_utils.parse_dockerimage_string(d)
            ...
            _logger.debug(
                "matchset: " +
                str([mregistry_rematch, mrepo_rematch, mtag_rematch]) + " : " +
                str([mregistry, mrepo, mtag]) + " : " +
                str([registry, repo, tag, tags]))

            if registry == mregistry or mregistry == '*' or re.match(
                    mregistry_rematch, registry):
                _logger.debug("checking mapping for image (" +
                              str(image_info) + ") match.")

                if repo == mrepo or mrepo == '*' or re.match(
                        mrepo_rematch, repo):
                    doit = False
                    matchstring = mname + ": N/A"
                    if tag:
                        if False and (mtag == tag or mtag == '*' or mtag
                                      in tags or re.match(mtag_rematch, tag)):
                            matchstring = mname + ":" + ','.join(
                                [mregistry, mrepo, mtag])
                            doit = True
                        else:
                            for t in tags:
                                if re.match(mtag_rematch, t):
                                    ...
                                    doit = True
                                    break
                    if not doit and (digest and
                                     (mdigest == digest or mdigest
                                      in in_digests or mdigest in digests)):
                        matchstring = mname + ":" + ','.join(
                            [mregistry, mrepo, mdigest])
                        doit = True

                    if not doit and (imageId and (mimageId == imageId)):
                        matchstring = mname + ":" + ','.join(
                            [mregistry, mrepo, mimageId])
                        doit = True

                    matchstring = matchstring.encode('utf8')
                    if doit:
                        _logger.debug("match found for image (" +
                                      str(image_info) + ") matchstring (" +
                                      str(matchstring) + ")")
                        ...
                        for wlname in wlnames:
                            wldataset = set(
                                list(wldataset) +
                                extract_whitelist_data(bundle, wlname))
                        ...
                        ret.append((poldata, wldata, polname, wlnames,
                                    matchstring, m, evalhash))
                        return (ret)
                    else:
                        _logger.debug("no match found for image (" +
                                      str(image_info) + ") match.")
                else:
                    _logger.debug("no match found for image (" +
                                  str(image_info) + ") match.")
...
def execute_gates(imageId, policies, refresh=True):
    ...
    success = True
    nextlinux_config = contexts['nextlinux_config']

    imagename = imageId
    gatesdir = '/'.join([nextlinux_config["scripts_dir"], "gates"])
    workingdir = '/'.join([nextlinux_config['nextlinux_data_dir'], 'querytmp'])
    ...
    nextlinux_utils.write_plainfile_fromstr(imgfile, imageId)

    try:
        gmanifest, failedgates = nextlinux_utils.generate_gates_manifest()
        if failedgates:
            _logger.error(
            ...
            for gatecheck in policies.keys():
                ...
                if gcommands:
                    for command in gcommands:
                        cmd = [command] + [
                            imgfile, nextlinux_config['image_data_store'],
                            outputdir
                        ] + params
                        _logger.debug("running gate command: " +
                                      str(' '.join(cmd)))

                        (rc, sout, cmdstring) = nextlinux_utils.run_command(cmd)
                        if rc:
    ...
    if success:
        ...
        contexts['nextlinux_db'].save_gates_report(imageId, report)
...
def generate_gates_report(imageId):
    ...
    outputs = contexts['nextlinux_db'].list_gate_outputs(imageId)
    for d in outputs:
        report[d] = contexts['nextlinux_db'].load_gate_output(imageId, d)

    return (report)


def evaluate_gates_results(imageId, policies, image_whitelist,
                           global_whitelist):
    ret = list()
    fullret = list()
    ...
    for m in policies.keys():
        gdata = contexts['nextlinux_db'].load_gate_output(imageId, m)
        for l in gdata:
            ...
            # nextlinux compatible json string) then extract the
            ...
            if k in policies[m]:
                ...
                elif image_whitelist and 'ignore' in image_whitelist and (
                        r in image_whitelist['ignore']):
                    whitelisted = True
                    ...
                    try:
                        for [gmod, gtriggerId] in global_whitelist:
                            if gmod == m:
                                ...
                                try:
                                    if gmod == 'NEXTLINUXSEC' and not re.match(
...
def structure_eval_results(imageId,
                           evalresults,
                           show_triggerIds=False,
    ...
    record['result']['header'] += [
        'Gate', 'Trigger', 'Check_Output', 'Gate_Action'
    ]
    if show_whitelisted:
...
if __name__ == '__main__':
    ...
    config = NextlinuxConfiguration(cliargs={})
    nextlinux_utils.nextlinux_common_context_setup(config)

    policies = {}
    whitelists = {}
    mappings = []

    pol0 = read_policy(name=str(uuid.uuid4()),
                       file='/root/.nextlinux/conf/nextlinux_gate.policy')
    pol1 = read_policy(name=str(uuid.uuid4()),
                       file='/root/.nextlinux/conf/nextlinux_gate.policy')
    ...
    map0 = create_mapping(map_name="default",
                          policy_name=policies.keys()[0],
                          whitelists=whitelists.keys(),
                          repotagstring='*/*:*')
    ...
    thebun = convert_to_policy_bundle(
        name='default',
        policy_file='/root/.nextlinux/conf/nextlinux_gate.policy',
        policy_version=default_policy_version,
        whitelist_files=['/root/wl0'],
        whitelist_version=default_whitelist_version)
    rc = write_policy_bundle(bundle_file="/tmp/bun1.json", bundle=thebun)

    pol0 = read_policy(name="meh",
                       file='/root/.nextlinux/conf/nextlinux_gate.policy')
    ...
    result, image_ecode = run_bundle(nextlinux_config=config,
                                     image='alpine',
                                     matchtags=[],
                                     bundle=thebun)
    ...
    try:
        result, image_ecode = run_bundle_stateless(nextlinux_config=config,
                                                   image='alpine',
                                                   matchtags=[],
                                                   bundle=thebun)
