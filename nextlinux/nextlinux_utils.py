...
import nextlinux_image, nextlinux_image_db
from configuration import NextlinuxConfiguration
from nextlinux.util import contexts, scripting
import nextlinux_auth
import nextlinux_feeds
...
def init_analyzer_cmdline(argv, name):
    ...
    nextlinux_conf = NextlinuxConfiguration()
    nextlinux_common_context_setup(nextlinux_conf)


    ret['analyzer_config'] = None
    nextlinux_analyzer_configfile = '/'.join([nextlinux_conf.config_dir, 'analyzer_config.yaml'])
    ...
    ret['nextlinux_config'] = nextlinux_conf.data
...
def init_gate_cmdline(argv, gate_name, gate_help={}):
    if len(argv) > 2 and argv[2] == 'nextlinux_get_help':
        if gate_help:
            if argv[1] != 'stdout':
...
def init_query_cmdline(argv, paramhelp):
    ...
    ret['image_report'] = contexts['nextlinux_db'].load_image_report(ret['imgid'])
...
def nextlinux_common_context_setup(config):
    ...
    if 'nextlinux_allimages' not in contexts or not contexts['nextlinux_allimages']:
        contexts['nextlinux_allimages'] = {}

    if 'nextlinux_db' not in contexts or not contexts['nextlinux_db']:
        contexts['nextlinux_db'] = nextlinux_image_db.load(driver=config['nextlinux_db_driver'], config=config)

    if 'nextlinux_auth' not in contexts or not contexts['nextlinux_auth']:
        aafile = os.path.join(config.config_dir, "nextlinux_auth.json")
        username = config.DEFAULT_ANON_NEXTLINUX_USERNAME
        password = config.DEFAULT_ANON_NEXTLINUX_PASSWORD
        ...
        contexts['nextlinux_auth'] = nextlinux_auth.nextlinux_auth_init(username, password, aafile, config['nextlinux_client_url'], config['nextlinux_token_url'], config['nextlinux_auth_conn_timeout'], config['nextlinux_auth_max_retries'])

    if 'nextlinux_config' not in contexts or not contexts['nextlinux_config']:
        contexts['nextlinux_config'] = config

    return(True)

def load_analyzer_config(nextlinux_conf_dir):
    nextlinux_analyzer_config = {}
    csum = None

    nextlinux_analyzer_configfile = '/'.join([nextlinux_conf_dir, 'analyzer_config.yaml'])
    if os.path.exists(nextlinux_analyzer_configfile):
        try:
            with open(nextlinux_analyzer_configfile, 'r') as FH:
                adata = FH.read()
                csum = hashlib.md5(adata).hexdigest()
                nextlinux_analyzer_config = yaml.safe_load(adata)
        except Exception as err:
            raise err

    return(nextlinux_analyzer_config, csum)

# nextlinuxDB pass through functions

def is_image_analyzed(imageId):
    return(contexts['nextlinux_db'].is_image_analyzed(imageId))

def del_files_cache(imageId, namespace=None):
    return(contexts['nextlinux_db'].del_files_cache(imageId, namespace=None))

def load_files_tarfile(imageId, namespace):
    return(contexts['nextlinux_db'].load_files_tarfile(imageId, namespace))

def load_files_metadata(imageId, namespace):
    return(contexts['nextlinux_db'].load_files_metadata(imageId, namespace))

def load_files_namespaces(imageId):
    return(contexts['nextlinux_db'].load_files_namespaces(imageId))

def save_files(imageId, namespace, rootfsdir, files):
    return(contexts['nextlinux_db'].save_files(imageId, namespace, rootfsdir, files))

def save_gate_output(imageId, gate_name, data):
    return(contexts['nextlinux_db'].save_gate_output(imageId, gate_name, data))

def save_gate_help_output(gate_help):
    return(contexts['nextlinux_db'].save_gate_help_output(gate_help))

def save_analysis_output(imageId, module_name, module_value, data, module_type=None):
    return(contexts['nextlinux_db'].save_analysis_output(imageId, module_name, module_value, data, module_type=module_type))


def load_analysis_output(imageId, module_name, module_value):
    ret = {}
    ret = contexts['nextlinux_db'].load_analysis_output(imageId, module_name, module_value, module_type='user')
    if ret: return(ret)
    ret = contexts['nextlinux_db'].load_analysis_output(imageId, module_name, module_value, module_type='extra')
    if ret: return(ret)
    ret = contexts['nextlinux_db'].load_analysis_output(imageId, module_name, module_value)
    if ret: return(ret)

    return(ret)

def load_gate_output(imageId, gate_name):
    return(contexts['nextlinux_db'].load_gate_report(imageId, gate_name))

def load_image_report(imageId):
    return(contexts['nextlinux_db'].load_image_report(imageId))

def load_analysis_report(imageId):
    return(contexts['nextlinux_db'].load_analysis_report(imageId))

def load_gates_report(imageId):
    return(contexts['nextlinux_db'].load_gates_report(imageId))

def load_gates_eval_report(imageId):
    return(contexts['nextlinux_db'].load_gates_eval_report(imageId))


def list_analysis_outputs(imageId):
    return(contexts['nextlinux_db'].list_analysis_outputs(imageId))

def load_analyzer_manifest(imageId):
    return(contexts['nextlinux_db'].load_analyzer_manifest(imageId))


def load_image(imageId):
    return(contexts['nextlinux_db'].load_image(imageId))

def load_all_images():
    return(contexts['nextlinux_db'].load_all_images())
...
def is_image_present(imageId, imagelist=None):
    return(contexts['nextlinux_db'].is_image_present(imageId, imagelist))


def get_image_list():
    return(contexts['nextlinux_db'].get_image_list())

def delete_image(imageId):
    return(contexts['nextlinux_db'].delete_image(imageId))
...
def is_intermediate_image(imageId, image_report=None):
    ...
    if image_report:
        try:
            if str(image_report['meta']['usertype']) in ['user', 'base', 'nextlinuxbase', 'oldnextlinuxbase']:
                return(False)
        except:
            pass

        try:
            if image_report['nextlinux_all_tags'] or image_report['nextlinux_current_tags']:
                return(False)
        except:
            pass
        ...
        raise Exception("could not load input image from nextlinuxDB: " + str(imageId))

    return(True)

def make_nextlinuxtmpdir(tmproot):
    tmpdir = '/'.join([tmproot, str(random.randint(0, 9999999)) + ".nextlinuxtmp"])
    try:
        os.makedirs(tmpdir)
        return(tmpdir)
    except:
...
def generate_gates_manifest():
    ...
    config = contexts['nextlinux_config']
    gmanifest = contexts['nextlinux_db'].load_gates_manifest()
...
#                pass
    ...
    for gdir in path_overrides + [gatesdir]:
        for gcmd in os.listdir(gdir):
            ...
            if script not in gmanifest or gmanifest[script]['csum'] == 'N/A' or gmanifest[script]['csum'] != csum or gmanifest[script]['returncode'] != 0:
                ...
                cmd = [script, 'stdout', "nextlinux_get_help"]
                try:
                    el['command'] = ' '.join(cmd)
    ...
    contexts['nextlinux_db'].save_gates_manifest(gmanifest)
...
def discover_gates_orig():
    ...
    gatesdir = '/'.join([config["scripts_dir"], "gates"])
    outputdir = make_nextlinuxtmpdir(config['tmpdir'])
    ...
    try:
        results = scripting.ScriptSetExecutor(path=gatesdir, path_overrides=path_overrides).execute(capture_output=True, fail_fast=True, cmdline=' '.join([outputdir, 'nextlinux_get_help']))
    except Exception as err:
        pass
...
def get_imageIds_named(name):
    ret = list()
    for result in contexts['nextlinux_db'].load_all_images_iter():
        imageId = result[0]
        image = result[1]
        if name == imageId:
            ret.append(imageId)
            ...
            ret.append(imageId)
        elif name in image['nextlinux_all_tags'] + image['nextlinux_current_tags'] or name+":latest" in image['nextlinux_all_tags'] + image['nextlinux_current_tags']:
            ret.append(imageId)
...
def discover_imageId(name):
    ...
    try:
        ...
        if not imageId:
            _logger.debug("looking for name ("+name+") in nextlinuxDB")
            for iname in name_variants:
                if contexts['nextlinux_db'].is_image_present(iname):
                    imageId = name
                    break

        if not imageId:
            _logger.debug("trying to load name ("+name+") from nextlinuxDB")
            for iname in name_variants:
                aimage = contexts['nextlinux_db'].load_image(iname)
                if aimage:
                    imageId = name
                    break

        if not imageId:
            _logger.debug("searching for name ("+name+") in nextlinuxDB")
            for iname in name_variants:
                ilist = get_imageIds_named(iname)
                if len(ilist) == 1:
                    imageId = ilist[0]
                    #aimage = contexts['nextlinux_db'].load_image(imageId)
                elif len(ilist) > 1:
                    raise ValueError("Input image name '"+str(iname)+"' is ambiguous in nextlinux:\n\tmatching imageIds: " + str(ilist))
    ...
    if not imageId:
        raise ValueError("Input image name '"+str(name)+"' not found in local dockerhost or nextlinux DB.")
...
def rpm_prepdb(unpackdir):
    ...
    if os.path.exists(origrpmdir):
        newrpmdirbase = make_nextlinuxtmpdir(unpackdir)
...
def get_distro_from_imageId(imageId):
    ...
    nextlinux_analyzer_meta = load_analysis_output(imageId, 'analyzer_meta', 'analyzer_meta')
    meta['DISTRO'] = nextlinux_analyzer_meta.pop('DISTRO', 'UNKNOWN')
    meta['DISTROVERS'] = nextlinux_analyzer_meta.pop('DISTROVERS', 'UNKNOWN')
    meta['LIKEDISTRO'] = nextlinux_analyzer_meta.pop('LIKEDISTRO', 'UNKNOWN')
    return (meta)
...
def cve_load_data(imageId, cve_data_context=None):
    ...
    for f in distrolist:
        ...
        if cve_data_context and dstr in cve_data_context:
            ...
            feeddata = nextlinux_feeds.load_nextlinux_feed('vulnerabilities', ':'.join([f[0], f[1]]), ensure_unique=True)
            if feeddata['success']:
    ...
    last_update = 0
    try:
        d = nextlinux_feeds.load_nextlinux_feed_group_datameta('vulnerabilities', dstr)
...
def image_context_add(imagelist, allimages, docker_cli=None, dockerfile=None, tmproot='/tmp', nextlinux_db=None, docker_images=None, must_be_analyzed=False, usertype=None, must_load_all=False):
    retlist = list()
    for i in imagelist:
        if i in allimages:
            retlist.append(i)
        elif must_be_analyzed and not nextlinux_db.is_image_analyzed(i):
            errorstr = "Image(s) must be analyzed before operation can be performed.\n\tImage: " + str(i)
            raise Exception(errorstr)
        else:
            try:
                newimage = nextlinux_image.NextlinuxImage(i, docker_cli=docker_cli, allimages=allimages, dockerfile=dockerfile, tmpdirroot=tmproot, usertype=usertype, nextlinux_db=nextlinux_db, docker_images=docker_images)
            except Exception as err:
                if must_load_all:
                    traceback.print_exc()
...
def diff_images(imageId, baseimageId):
    ...
    areport = contexts['nextlinux_db'].load_analysis_report(imageId)
    breport = contexts['nextlinux_db'].load_analysis_report(baseimageId)
