...
from nextlinux.util import scripting
...
class SelectionStrategy(object):
    ...
    def evaluate_familytree(self, family_tree, image_set):
        ...
        if family_tree is None or image_set is None:
            raise ValueError(
                'Cannot execute analysis strategy on None image or image with no familytree data'
            )
        ...
        for i in family_tree:
            image = image_set[i]
            if self._should_analyze_image(image, family_tree.index(i),
                                          tree_len):
    ...
    def _should_analyze_image(self, image, pos, count):
        ...
        :param image: NextlinuxImage object
...
class Analyzer(object):
    ...
    def __init__(self, nextlinux_config, imagelist, allimages, force, args=None):
        ...
        try:
            if 'isbase' in args and args['isbase']:
                usertype = 'base'
            elif 'nextlinuxbase' in args and args['nextlinuxbase']:
    ...
    def list_analyzers(self):
        ...
        if not os.path.exists(analyzerdir):
            raise Exception(
                "No base analyzers found - please check nextlinux insallation for completeness"
            )
        else:
    ...
    def run_analyzers(self, image):
        ...
        #outputdir = image.nextlinux_imagedir
        shortid = image.meta['shortId']
        imagedir = None

        analyzer_status = self.nextlinuxDB.load_analyzer_manifest(
            image.meta['imageId'])

        analyzer_config = {}
        analyzer_config_csum = None
        try:
            analyzer_config, analyzer_config_csum = nextlinux_utils.load_analyzer_config(
        ...
        if 'analyzer_config_csum' in analyzer_status:
            try:
                if analyzer_status['analyzer_config_csum'][
                        'csum'] != analyzer_config_csum:
                    self._logger.debug(
                        "nextlinux analyzer config has been updating, forcing re-analysis"
                    )
                    self.force = True
                    analyzer_status['analyzer_config_csum'][
                        'csum'] = analyzer_config_csum
            except:
                pass
        else:
                        ...
                        if os.path.exists(
                            ...
                            for dd in os.listdir(
                                ...
                                if os.path.isfile(dfile):
                                    adata = nextlinux_utils.read_kvfile_todict(
                                        dfile)
                                    self.nextlinuxDB.save_analysis_output(
                                        image.meta['imageId'],
                                        module_name,
                                        module_value,
                                        adata,
        ...
        self.nextlinuxDB.save_analyzer_manifest(image.meta['imageId'],
                                              analyzer_status)
        ...
        if success:
            ...
            if didsave:
                ...
                self.nextlinuxDB.save_analysis_report(image.meta['imageId'],
                                                    report)
    ...
    def generate_analysis_report(self, image):
        ...
        report = {}
        amanifest = self.nextlinuxDB.load_analyzer_manifest(
            image.meta['imageId'])
        for amodule in amanifest.keys():
            if 'analyzer_outputs' in amanifest[amodule]:
                for aoutput in amanifest[amodule]['analyzer_outputs']:
                    ...
                    if data_type == 'file':
                        adata = self.nextlinuxDB.load_analysis_output(
                            image.meta['imageId'],
                            module_name,
                            module_value,
    ...
    def run(self):
        self._logger.debug("main image analysis on images: " +
                           str(self.images) + ": begin")
        ...
        for imageId in self.images:
            ...
            toanalyze.update(
                self.selection_strategy.evaluate_familytree(
                    coreimage.nextlinux_familytree, self.allimages))
        ...
        for imageId in toanalyze.keys():
            ...
            if not success:
                self._logger.error("analyzer failed to run on image " +
                                   str(image.meta['imagename']) +
                                   ", skipping the rest")
                break
        ...
        #    for imageId in toanalyze.keys():
        #        c = controller.Controller(nextlinux_config=self.config, imagelist=[imageId], allimages=self.allimages).run_gates(refresh=True)
