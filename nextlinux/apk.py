...
def next_token(expected_type, data):
    ...
    elif (expected_type == TokenType.DIGIT or expected_type
          == TokenType.DIGIT_OR_ZERO) and next_data[0].isalpha():
    ...
    if next_type < expected_type:
        if not ((next_type == TokenType.DIGIT_OR_ZERO
                 and expected_type == TokenType.DIGIT) or
                (next_type == TokenType.SUFFIX
                 and expected_type == TokenType.SUFFIX_NO) or
                (next_type == TokenType.DIGIT
                 and expected_type == TokenType.LETTER)):
            next_type = TokenType.INVALID
...
def get_token(expected_type, data):
    ...
    if expected_type == TokenType.DIGIT_OR_ZERO and data[i] == '0':
        ...
        token_value = -i
            ...
            TokenType.DIGIT_OR_ZERO, TokenType.DIGIT, TokenType.SUFFIX_NO,
