...
class NextlinuxImageDB(object):
    ...
    def __init__(self, config={}):
        ...
        self.version = None
        self.nextlinux_version_string = None
        self.nextlinux_db_version_string = None
        try:
            from nextlinux import version as nextlinux_version_string
            self.nextlinux_version_string = nextlinux_version_string
            self.nextlinux_db_version_string = self.nextlinux_version_string
            try:
                patt = re.match("([0-9]+)\.([0-9]+)\.(.*)",
                                self.nextlinux_version_string)
                if patt:
                    (major, minor, subminor) = patt.group(1, 2, 3)
                    self.nextlinux_db_version_string = '.'.join([major, minor])
            except Exception as err:
                pass

            self.version = {
                'nextlinux_version': self.nextlinux_version_string,
                'db_version': self.nextlinux_db_version_string
            }
    ...
    def is_image_present(self, imageId, imagelist=None):
        self._logger.debug("NextlinuxDB: parent unimplemented")
        return (False)

    def is_image_analyzed(self, imageId):
        self._logger.debug(
            "NextlinuxDB: parent unimplemented: driver has not implemented this nextlinuxDB operation"
        )
        return (False)
