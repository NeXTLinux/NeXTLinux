...
import nextlinux_image_db_base

def load(config={}):
    return(NextlinuxImageDB_FS(config=config))

class NextlinuxImageDB_FS(nextlinux_image_db_base.NextlinuxImageDB):
    _db_metadata_filename = 'nextlinux_db_meta.json'

    def __init__(self, config={}):
        nextlinux_image_db_base.NextlinuxImageDB.__init__(self, config)
        if not self.config:
            raise Exception("invalid config passed to driver init")

        try:
            dbconfig = config['nextlinux_db_driver_config']
            try:
        ...
        try:
            basedir = config['nextlinux_data_dir']
        except:
            basedir = None
        ...
        # for policy bundles from nextlinux.io
        ...
        self._logger.debug("using directory for nextlinux image data: " + str(imagerootdir))
        self._logger.debug("using directory for nextlinux feed data: " + str(feedrootdir))
        self._logger.debug("using directory for nextlinux policy data: " + str(policyrootdir))
        ...
        try:
            for d in [self.imagerootdir, self.feedrootdir, self.policyrootdir]:
                if not os.path.exists(d):
                    try:
                        ...
                        raise Exception("nextlinux DB: data dir does not exist and cannot be created "+str(d))

            # check that the software and DB are same version before continuing
            dbmetafile = '/'.join([self.imagerootdir, "nextlinux_db_meta.json"])
            ...
            if 'nextlinux_version' not in json_dict:
                json_dict['nextlinux_version'] = self.nextlinux_version_string
                update = True
            elif json_dict['nextlinux_version'] != self.nextlinux_version_string:
                json_dict['nextlinux_version'] = self.nextlinux_version_string
                update = True

            if 'nextlinux_db_version' not in json_dict:
                json_dict['nextlinux_db_version'] = self.nextlinux_db_version_string
                update = True
            elif json_dict['nextlinux_db_version'] != self.nextlinux_db_version_string:
                # Check major version
                local_db_version = json_dict['nextlinux_db_version']
                db_version_tuple = tuple(local_db_version.split('.'))
                code_version_tuple = tuple(self.nextlinux_db_version_string.split('.'))
                if db_version_tuple[0] < code_version_tuple[0]:
                    raise ValueError("Nextlinux DB version ("+local_db_version + ") cannot be automatically upgraded to this Nextlinux software version (" + self.nextlinux_version_string + ") Please downgrade Nextlinux to compatible version or remove your Nextlinux DB and start from a clean install.")
                else:
                    json_dict['nextlinux_db_version'] = self.nextlinux_db_version_string
                    update = True
    ...
    def exists(path, with_expected_db_version=None):
        ...
        :return:
        """
        dbmetafile = os.path.join(path, NextlinuxImageDB._db_metadata_filename)
        ...
        if with_expected_db_version:
            ...
            if json_dict['nextlinux_db_version'] != with_expected_db_version:
                return False
        else:
            return True
    ...
    def get_image_list(self):
        ...
        for d in os.listdir(self.imagerootdir):
            if os.path.exists(os.path.join(self.imagerootdir, d, "analyzers.done")):
                ...
                ret[d] = list(set(t['nextlinux_all_tags']) | set(t['nextlinux_current_tags']))
        return(ret)
    ...
    def save_image_new(self, imageId, report=None):
        ...
        nextlinux_utils.write_kvfile_fromdict(thefile, report['image_report']['meta'])

        thefile = os.path.join(thedir, "Dockerfile")
        nextlinux_utils.write_plainfile_fromstr(thefile, report['image_report']['dockerfile_contents'])
    ...
    def save_analysis_report(self, imageId, report):
        ...
        nextlinux_utils.update_file_jsonstr(json.dumps(report), thefile, False)
    ...
    def load_analysis_output(self, imageId, module_name, module_value, module_type=None):
        ...
        if os.path.exists(thefile):
            if os.path.isfile(thefile):
                ret = nextlinux_utils.read_kvfile_todict(thefile)
    ...
    def save_analysis_output(self, imageId, module_name, module_value, data, module_type=None, directory_data=False):
        ...
        if not directory_data:
            ...
            return(nextlinux_utils.write_kvfile_fromdict(thefile, data))
        else:
    ...
    def load_gate_output(self, imageId, gate_name):
        ...
        return(nextlinux_utils.read_plainfile_tolist(thefile))
    ...
    def save_gate_output(self, imageId, gate_name, data):
        ...
        if not os.path.exists(thedir):
            os.makedirs(thedir)
        thefile = os.path.join(thedir, gate_name)
        return(nextlinux_utils.write_plainfile_fromlist(thefile, data))
    ...
    def save_gate_help_output(self, gate_help):
        ...
        rc = nextlinux_utils.update_file_jsonstr(json.dumps(gate_help), thefile)
        return(rc)
    ...
    def load_gate_policy(self, imageId):
        thefile = os.path.join(self.imagerootdir, imageId, 'nextlinux_gate.policy')
        return(nextlinux_utils.read_plainfile_tolist(thefile))
    ...
    def save_gate_policy(self, imageId, data):
        ...
        if not os.path.exists(thedir):
            os.makedirs(thedir)
        thefile = os.path.join(thedir, "nextlinux_gate.policy")
        return(nextlinux_utils.write_plainfile_fromlist(thefile, data))
    ...
    def load_gate_whitelist(self, imageId):
        thefile = os.path.join(self.imagerootdir, imageId, 'nextlinux_gate.whitelist')
        return(nextlinux_utils.read_plainfile_tolist(thefile))
    ...
    def save_gate_whitelist(self, imageId, data):
        ...
        if not os.path.exists(thedir):
            os.makedirs(thedir)
        thefile = os.path.join(thedir, 'nextlinux_gate.whitelist')
        return(nextlinux_utils.write_plainfile_fromlist(thefile, data))
    ...
    def save_image_report(self, imageId, report):
        ...
        thefile = os.path.join(thedir, "image.meta")
        if 'meta' in report:
            nextlinux_utils.write_kvfile_fromdict(thefile, report['meta'])
        ...
        if os.path.exists(thefile):
            ...
            if 'nextlinux_current_tags' not in oldreport:
                oldreport['nextlinux_current_tags'] = list([date, report['nextlinux_current_tags']])

            diff = list(set(oldreport['nextlinux_current_tags']).symmetric_difference(set(report['nextlinux_current_tags'])))
            if len(diff) > 0:
                # there is a difference between stored tags and new tags
                report['tag_history'].append([date, oldreport['nextlinux_current_tags']])
        
        if 'tag_history' not in report:
            report['tag_history'] = list()

        if len(report['tag_history']) <= 0:
            report['tag_history'].append([date, report['nextlinux_current_tags']])
    ...
    def load_files_metadata(self, imageId, namespace):
        ...
        if not os.path.exists(thefile):
            ...
            allfiles = nextlinux_utils.get_files_from_tarfile(thefile)
