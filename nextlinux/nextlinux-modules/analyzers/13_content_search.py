...
if 'analyzer_config' in config and config['analyzer_config']:
    if 'regexp_match' in config['analyzer_config'] and type(
            config['analyzer_config']['regexp_match']) == list:
        regexps = config['analyzer_config']['regexp_match']
    if 'match_params' in config['analyzer_config'] and type(
            config['analyzer_config']['match_params']) == list:
...
if matchparams:
    for param in matchparams:
        try:
            ...
            print "WARN: could not parse parameter (should be 'key=value'), ignoring: " + str(
                param)
...
for name in allfiles.keys():
    ...
    if os.path.isfile(thefile):
        ...
        dochecks = True
        if params['maxfilesize'] and os.path.getsize(
                thefile) > params['maxfilesize']:
            dochecks = False
        else:
            try:
                fmimetype = "unknown"
            ...
            if fmimetype != 'unknown' and (params['mimetypefilter']
                                           and fmimetype
                                           not in params['mimetypefilter']):
                dochecks = False
                ...
                for line in FH.readlines():
                    for regexp in regexps:
                        ...
                        try:
                            ...
                            import traceback
                            traceback.print_exc()
                            print "ERROR: configured regexp not valid or regexp cannot be applied - exception: " + str(
                                err)
                            sys.exit(1)
                    lineno += 1
        else:
...
if outputdata:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, outputdata)

if params['storeonmatch'] and storefiles:
    nextlinux.nextlinux_utils.save_files(imageId, analyzer_name, rootfsdir,
                                     storefiles)
