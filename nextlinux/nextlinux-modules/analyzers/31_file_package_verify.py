...
def apk_get_file_package_metadata(unpackdir, record_template):
    ...
    apkdbpath = os.path.join(unpackdir, 'rootfs', 'lib', 'apk', 'db',
                             'installed')
    try:
...
def deb_get_file_package_metadata(unpackdir, record_template):
    ...
    result = {}
    conffile_csums = {}
    statuspath = os.path.join(unpackdir, "rootfs", "var", "lib", "dpkg",
                              "status")
                    ...
                    if re.match("^Conffiles:.*", line):
                        ...
                        if fmode:
                            try:
                                ...
                                print "WARN: bad line in status for conffile line - exception: " + str(
                                    err)
    ...
    try:
        if os.path.exists(metapath):
            ...
            raise Exception("no dpkg info path found in image: " +
                            str(metapath))
        ...
        for pkg in metafiles.keys():
            ...
            try:
                ...
                print "WARN: could not open/read metafile - exception: " + str(
                    err)
            ...
            if dinfo:
                for line in dinfo.splitlines():
                    ...
                    try:
                        ...
                        print "WARN: problem parsing line from dpkg info file - exception: " + str(
                            err)
        ...
        for pkg in conffiles.keys():
            ...
            try:
                ...
                cinfo = None
                print "WARN: could not open/read conffile - exception: " + str(
                    err)
            ...
            if cinfo:
                for line in cinfo.splitlines():
                    ...
                    try:
                        ...
                        print "WARN: problem parsing line from dpkg conffile file - exception: " + str(
                            err)
    ...
    try:
        ...
        if exitcode == 0:
            for l in soutput.splitlines():
                ...
                if l:
                    try:
                        (fname, fdigest, fmode, fgroup, fuser, fsize, fpackage,
                         fflags, fdigestalgonum) = l.split("|ANCHORETOK|")
                        fname = re.sub('""', '', fname)
                        cfile = False
                        if 'c' in str(fflags):
                            cfile = True
                        ...
                        try:
                            fdigestalgo = rpm_digest_algo_map[int(
                                fdigestalgonum)]
                        except:
                            fdigestalgo = 'unknown'
                        ...
                        print "WARN: unparsable output line - exception: " + str(
                            err)
        else:
            raise Exception(
                "rpm file metadata command failed with exitcode (" +
                str(exitcode) + ") - stdoutput: " + str(soutput) +
                " : stderr: " + str(serror))

    except Exception as err:
        raise Exception(
            "WARN: distro package metadata gathering failed - exception: " +
            str(err))
...
if resultlist:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, resultlist)

# now run the distro package verifier, if present

verify_result = {}
try:
    vhash, vcmd, voutput, verror, vexitcode = nextlinux.nextlinux_utils.verify_file_packages(
        unpackdir, flavor)
    if vcmd:
        verify_result = {
            'cmd': vcmd,
    ...
    print "WARN: could not run distro package verifier - exception: " + str(
        err)
    verify_result = {}
...
if verify_result:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, verify_output)
