...
if 'analyzer_config' in config and config['analyzer_config']:
    if 'file_list' in config['analyzer_config'] and type(
            config['analyzer_config']['file_list']) == list:
...
try:
    nextlinux.nextlinux_utils.save_files(imgid, analyzer_name, rootfsdir,
                                     storefiles)
except Exception as err:
