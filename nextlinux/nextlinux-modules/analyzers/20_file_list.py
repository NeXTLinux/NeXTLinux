...
def rpm_check_file_membership_from_path(unpackdir, allfiles=None):
    ...
    if not allfiles:
        filemap, allfiles = nextlinux.nextlinux_utils.get_files_from_path(inpath)
    ...
    try:
        ...
        while not done:
            ...
            if len(cmdlist) <= 0:
                ...
                try:
                    sout = subprocess.check_output(
                        ['rpm', '--dbpath=' + rpmdbdir, '-qf'] + cmdlist,
...
def dpkg_check_file_membership_from_path(unpackdir, allfiles=None):
    ...
    try:
        try:
            ...
            for flist in nextlinux.nextlinux_utils.grouper(allfiles.keys(), 256):
                try:
                    sout = subprocess.check_output([
                        'dpkg', "--admindir=" + unpackdir +
                        "/rootfs/var/lib/dpkg", '-S'
                    ] + flist,
                                                   stderr=subprocess.STDOUT)
                    sout = sout.decode('utf8')
                except subprocess.CalledProcessError as err:
...
try:
    ...
    if distrodict['flavor'] == "RHEL":
        # rpm file check
        match, nonmatch = rpm_check_file_membership_from_path(
            unpackdir, allfiles=allfiles)
        for f in nonmatch:
            nonpkgoutfiles[f] = 'NOTPKGED'
    elif distrodict['flavor'] == "DEB":
        # dpkg file check
        match, nonmatch = dpkg_check_file_membership_from_path(
            unpackdir, allfiles=allfiles)
        for f in nonmatch:
            nonpkgoutfiles[f] = 'NOTPKGED'
...
if simplefiles:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, simplefiles)
...
if nonpkgoutfiles:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, nonpkgoutfiles)
