...
try:
    timer = time.time()
    (tmp, allfiles) = nextlinux.nextlinux_utils.get_files_from_path(unpackdir +
                                                                "/rootfs")
...
if outfiles_sha1:
    ...
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, outfiles_sha1)

if outfiles_md5:
    ofile = os.path.join(outputdir, 'files.md5sums')
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, outfiles_md5)

if outfiles_sha256:
    ofile = os.path.join(outputdir, 'files.sha256sums')
    nextlinux.nextlinux_utils.write_kvfile_fromdict(ofile, outfiles_sha256)
