...
try:
    ...
    if os.path.exists(hfile):
        ...
        for record in history:
            ...
            clean_createdBy = re.sub(r"^/bin/sh -c #\(nop\) ", "",
                                     record['CreatedBy'])
            line = {
                'layer': clean_layer,
                'dockerfile_line': clean_createdBy,
                'layer_sizebytes': str(record['Size'])
            }
            output.append(line)
    else:
        raise Exception("nextlinux failed to provide file '" + str(hfile) +
...
nextlinux.nextlinux_utils.write_kvfile_fromdict(
