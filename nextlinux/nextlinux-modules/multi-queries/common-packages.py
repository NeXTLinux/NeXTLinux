...
def get_next(image, allimages):
    ...
    if i:
        nimage = nextlinux.nextlinux_image.NextlinuxImage(i, allimages=allimages)
...
for imageId in config['images']:
    image = nextlinux.nextlinux_image.NextlinuxImage(imageId, allimages=allimages)
...
for t in tups:
    ...
    try:
        diffdata = nextlinux.nextlinux_utils.diff_images(image.meta['imageId'], d)
...
for k in pkgout.keys():
    for p in pkgout[k].keys():
        if 'all' in config['params'] or k in config['params'] or k[
                0:12] in config['params']:
