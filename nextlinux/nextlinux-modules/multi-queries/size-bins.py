...
for name in config['params']:
    ...
    try:
        ...
        warns.append([
            "Invalid query arguments specified, using defaults.  Input must be 'default' or a list of increasing integers (bin maximums: ex '10 20 50 100 1000')."
        ])
...
for imageId in config['images']:
    try:
        image_data = nextlinux.nextlinux_utils.load_image(imageId)
        ...
        warns.append([
            "cannot calculate size for image: " + imageId + ": error: " +
            str(err)
        ])
...
for bstr in result.keys():
    if result[bstr]['imageIds']:
        ...
        outlist.append([
            bstr,
            str(result[bstr]['count']), ' '.join(result[bstr]['imageIds'])
        ])
    else:
...
if len(warns) > 0:
    nextlinux.nextlinux_utils.write_kvfile_fromlist(config['output_warns'], warns)
