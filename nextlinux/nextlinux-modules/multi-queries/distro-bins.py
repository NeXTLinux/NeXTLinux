...
for imageId in config['images']:
    try:
        image = nextlinux.nextlinux_image.NextlinuxImage(imageId,
                                                   allimages=allimages)
        ...
        result[distro][distro_vers][
            'count'] = result[distro][distro_vers]['count'] + 1
        result[distro][distro_vers]['imageIds'].append(image.meta['imageId'])

    except Exception as err:
        warns.append([
            "Failed to read distro/version from image: " +
            image.meta['imageId']
        ])
...
for d in result.keys():
    for v in result[d].keys():
        if not onlysummarize:
            ...
            outlist.append([
                d, v,
                str(result[d][v]['count']), ' '.join(result[d][v]['imageIds'])
            ])
...
if onlysummarize:
    for k in sums.keys():
        ...
        outlist.append(
            [k, 'any',
             str(sums[k]['count']), ' '.join(sums[k]['imageIds'])])
