...
import fs_util
from nextlinux.util import RFC1123_TIME_FORMAT
...
def normalize_headers(headers):
    ...
    if 'last-modified' in headers:
        ...
        result['last-modified'] = datetime.datetime.utcnow().strftime(
            RFC1123_TIME_FORMAT)

    # Normalize date
    if isinstance(result['last-modified'], datetime.datetime):
        result['last-modified'] = result['last-modified'].strftime(
            RFC1123_TIME_FORMAT)
...
class ResourceCache(object):
    ...
    def is_stale(self, url):
        self._logger.debug('Checking staleness of url %s into resource cache' %
                           url)
        ...
        try:
            if url in self.metadata:
                ...
                if 'etag' in headers:
                    ...
                    return self.metadata[url]['lastmodified'] != headers[
                        'lastmodified']

            else:
                return True
        except Exception, e:
            self._logger.debug('Error checking staleness of %s: %s' %
                               (url, e.message))
            raise
    ...
    def _load(self, url):
        ...
        try:
            if url in self.metadata:
                headers = retriever.fetch(content_path,
                                          last_etag=self.metadata[url]['etag'],
                                          progress_bar=self.progress_bars)
                if headers is None:
                    # no update, return what is already loaded
                    self._logger.info(
                        'Cached %s is up-to-date. No data download needed' %
                        url)
                    return self.metadata[url]
            else:
                headers = retriever.fetch(content_path,
