...
#from nextlinux import nextlinux_utils#, nextlinux_policy
import nextlinux_policy, nextlinux_utils

import logging

from nextlinux.util import scripting, contexts
...
class Controller(object):
    ...
    def __init__(self, nextlinux_config, imagelist, allimages, force=False):
        self.config = nextlinux_config
        self.allimages = allimages

        if len(imagelist) <= 0:
            raise Exception("No images given to evaluate")

        self.images = nextlinux_utils.image_context_add(
            imagelist,
            allimages,
            docker_cli=contexts['docker_cli'],
            tmproot=self.config['tmpdir'],
            nextlinux_db=contexts['nextlinux_db'],
            docker_images=contexts['docker_images'],
            must_be_analyzed=True,
            must_load_all=True)

        self.nextlinuxDB = contexts['nextlinux_db']

        self.default_gatepol = '/'.join(
            [self.config.config_dir, "nextlinux_gate.policy"])
        self.default_global_whitelist = os.path.join(
            self.config.config_dir, "nextlinux_global.whitelist")
        ...
        for k in policy.keys():
            for c in policy[k].keys():
                if policy[k][c]['params']:
                    outline = k + ":" + c + ":" + policy[k][c][
                        'action'] + ":" + policy[k][c]['params']
                else:
                    outline = k + ":" + c + ":" + policy[k][c]['action']
                outlist.append(outline)

        self.nextlinuxDB.save_gate_policy(imageId, outlist)
        return (True)
    ...
    def get_image_policies(self, image):
        ...
        try:
            policy = nextlinux_policy.read_policy(name='default',
                                                file=self.default_gatepol)
            policy_data = policy['default']
        except Exception as err:
            policy_data = []
        default_policies = nextlinux_policy.structure_policy(policy_data)

        policy_data = self.nextlinuxDB.load_gate_policy(image.meta['imageId'])
        image_policies = nextlinux_policy.structure_policy(policy_data)
    ...
    def load_global_whitelist(self):
        ...
        if self.global_whitelist_override and os.path.exists(
                self.global_whitelist_override):
            whitelist_file = self.global_whitelist_override
        elif self.default_global_whitelist and os.path.exists(
                self.default_global_whitelist):
            whitelist_file = self.default_global_whitelist
        else:
            self._logger.debug("no global whitelist can be found, skipping")

        if whitelist_file:
            whitelist_data = nextlinux_policy.read_whitelist(name='default',
                                                           file=whitelist_file)
            ret = nextlinux_policy.structure_whitelist(whitelist_data['default'])
    ...
    def load_whitelist(self, image):
        ...
        data = self.nextlinuxDB.load_gate_whitelist(image.meta['imageId'])
        if not data:
            return (ret)
    ...
    def save_whitelist(self, image, loaded, latest):
        ...
        whitelist = self.nextlinuxDB.load_gate_whitelist(image.meta['imageId'])
        if not whitelist:
            outlist = list()
            for i in latest:
            ...
            self.nextlinuxDB.save_gate_whitelist(image.meta['imageId'], outlist)
        else:
            newpol = False
            for i in latest:
                if 'check' in i and i['check'] == 'FINAL':
            ...
            if newpol:
                ...
                for i in new['enforce']:
                    outlist.append(json.dumps(i))
                self.nextlinuxDB.save_gate_whitelist(image.meta['imageId'],
                                                   outlist)
        return (True)
    ...
    def load_policies(self, image):
        ...
        if self.policy_override:
            ...
            policies = nextlinux_policy.structure_policy(policy_data)
        else:
    ...
    def evaluate_gates_results(self, image):
        ...
        ret, fullret = nextlinux_policy.evaluate_gates_results(
            image.meta['imageId'], policies, policies_whitelist,
            global_whitelist)

        # save the results
        self.save_whitelist(image, policies_whitelist, ret)
        for i in ret:
            self.nextlinuxDB.del_gate_eval_output(image.meta['imageId'],
                                                i['check'])
        ...
        for i in evals.keys():
            self.nextlinuxDB.save_gate_eval_output(image.meta['imageId'], i,
                                                 evals[i])

        self.nextlinuxDB.save_gates_eval_report(image.meta['imageId'], ret)
        return (ret, fullret)
    ...
    def execute_gates(self, image, refresh=True):
        ...
        success = nextlinux_policy.execute_gates(imageId, policies)
        if success:
            report = self.generate_gates_report(image)
            self.nextlinuxDB.save_gates_report(image.meta['imageId'], report)
    ...
    def generate_gates_report(self, image):
        ...
        outputs = self.nextlinuxDB.list_gate_outputs(image.meta['imageId'])
        for d in outputs:
            report[d] = self.nextlinuxDB.load_gate_output(
                image.meta['imageId'], d)

        return (report)

    def result_get_highest_action(self, results):
        return (nextlinux_policy.result_get_highest_action(results))
    ...
    def run_gates(self,
        ...
        for imageId in self.images:
            ...
            record = nextlinux_policy.structure_eval_results(
                imageId,
                fullresults,
                show_triggerIds=self.show_triggerIds,
                show_whitelisted=self.show_whitelisted,
                imageName=image.get_human_name())
    ...
    def listpolicy(self):
        ...
        for imageId in self.images:
            if imageId in self.allimages:
                policy_data = self.nextlinuxDB.load_gate_policy(imageId)
                image_pol = nextlinux_policy.structure_policy(policy_data)
    ...
    def rmpolicy(self):
        for imageId in self.images:
            if imageId in self.allimages:
                self.nextlinuxDB.del_gate_policy(imageId)

        return (True)

    def updatepolicy(self, newpolicyfile):
        #policy_data = nextlinux_utils.read_plainfile_tolist(newpolicyfile)
        try:
            policy = nextlinux_policy.read_policy(name='default',
                                                file=newpolicyfile)
            policy_data = policy['default']
        ...
        newpol = nextlinux_policy.structure_policy(policy_data)
        for imageId in self.images:
            if imageId in self.allimages:
                try:
    ...
    def edit_policy_file(self, editpolicy=False, whitelist=False):
        ...
        for imageId in self.images:
            if editpolicy:
                data = self.nextlinuxDB.load_gate_policy(imageId)
            else:
                data = self.nextlinuxDB.load_gate_whitelist(imageId)
            ...
            if not data:
                ...
                tmpdir = nextlinux_utils.make_nextlinuxtmpdir("/tmp")
                try:
                    thefile = os.path.join(tmpdir, "nextlinuxpol." + imageId)
                    nextlinux_utils.write_plainfile_fromlist(thefile, data)
                    ...
                    #newdata = nextlinux_utils.read_plainfile_tolist(thefile)
                    try:
                        policy = nextlinux_policy.read_policy(name='default',
                                                            file=thefile)
                    ...
                    if editpolicy:
                        self.nextlinuxDB.save_gate_policy(imageId, newdata)
                    else:
                        self.nextlinuxDB.save_gate_whitelist(imageId, newdata)
                except Exception as err:
                    pass
                finally:
                    if tmpdir:
